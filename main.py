...
class IntentClassifier:
    ...
    def __init__(self, model_name: str = "bert-base-uncased", use_gpu: bool = True):
        ...
        print(
            f"✓ Intent classifier loaded (device: {'GPU' if self.use_gpu else 'CPU'})"
        )
    ...
    def classify(self, text: str, top_k: int = 1) -> Tuple[str, float]:
        ...
        """
        result = self.pipeline(text, self.categories, multi_class=False, top_k=top_k)
...
class EmbeddingModel:
    ...
    def __init__(
        ...
        self.model = SentenceTransformer(
            model_name, device="cuda" if self.use_gpu else "cpu"
        )
    ...
    def embed(self, text: str, normalize: bool = True) -> List:
        """
...
class SemanticRouter:
    ...
    def route(self, query: str) -> Dict:
        ...
        best_category = (
            ...
            else max(similarities, key=lambda x: similarities[x])
        )
...
def print_result(result: Dict):
    ...
    print(
        f"   (Intent: {result['intent_confidence']:.1%}, Semantic: {result['semantic_confidence']:.1%})"
    )
